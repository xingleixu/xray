...
# 编译测试程序
test: $(TEST_TARGET) $(TEST_FOR_TARGET) $(TEST_AST_TARGET) $(TEST_PARSER_TARGET) $(TEST_EVAL_TARGET) $(TEST_VARIABLES_TARGET) $(TEST_CONTROL_FLOW_TARGET) $(TEST_FUNCTIONS_TARGET)
	@echo "运行词法分析器测试..."
	./$(TEST_TARGET)
	@echo ""
	@echo "运行 for 循环测试..."
...
# 清理
clean:
	rm -f $(CORE_OBJS) $(MAIN_OBJ) $(TEST_OBJ) $(TEST_FOR_OBJ) $(TEST_AST_OBJ) $(TEST_PARSER_OBJ) $(TEST_EVAL_OBJ) $(TEST_VARIABLES_OBJ) $(TEST_CONTROL_FLOW_OBJ) $(TEST_FUNCTIONS_OBJ) $(TARGET) $(TEST_TARGET) $(TEST_FOR_TARGET) $(TEST_AST_TARGET) $(TEST_PARSER_TARGET) $(TEST_EVAL_TARGET) $(TEST_VARIABLES_TARGET) $(TEST_CONTROL_FLOW_TARGET) $(TEST_FUNCTIONS_TARGET)
	@echo "清理完成"
