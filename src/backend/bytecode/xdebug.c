...
/*
** AsBC格式指令（B是有符号立即数，C是条件标志k）
** 用于比较立即数指令：LTI, LEI, GTI, GEI
*/
static int cmp_imm_instruction(const char *name, Proto *proto, int offset) {
    Instruction inst = proto->code[offset];
    uint8_t ra = GETARG_A(inst);
    int sb = GETARG_sB(inst);   /* 有符号B参数（立即数）*/
    uint8_t k = GETARG_C(inst);  /* 条件标志 */
...
int xr_disassemble_instruction(Proto *proto, int offset) {
    ...
    switch (op) {
        ...
        case OP_GTI:
        case OP_GEI:
            return cmp_imm_instruction(name, proto, offset);
